...
void LogReader:: backup(void)
    ...
    cout << "备份日志文件开始..." << endl;
    // 生成备份文件名
    time_t now = time(NULL);  // 1970-1-1 00：00：00  秒数
    ...
    // 生成备份指令
    string cmd = "cp " +m_logFile + " " + m_backupFile;
    cout << "备份指令:" << cmd << endl;
...
void LogReader::readLoginsFile (void)
    ...
    if(ifs){
        ...
        if(! ifs.eof())
            throw ReadException("登录文件无法读取");
        ifs.close();
        unlink(m_loginsFile.c_str());
    }
    cout << "登入文件记录：" << m_logins.size() << endl;
    cout << "读取登入文件完成。" << endl;
}
// 读取备份文件
void LogReader::readBackupFile (void) 
    ...
    for(int i=0;i<records;++i){
        ...
        pid = ntohl(pid);//日志中的多字节整数都是网络字节序
        // 登录类型
        short type;
